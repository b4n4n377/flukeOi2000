 ...
 * the end of the file for an example for running f9010.
 *
...
//reads from 0000-ffff  
#define Z80_READ_BYTE(address, x)                                       \
{                                                                       \
        (x) = ((F9010 *) context)->memory[(address) & 0xffff];	\
}
...
//writes: Ram from C000 - FFFF
  
#define Z80_WRITE_BYTE(address, x)                                      \
{                                                                       \
  ((F9010 *) context)->memory[((address) & 0xffff) | 0xc000] = (x);	\
}
...
{                                                                       \
	unsigned char	*memory;					\
									\
	memory = ((F9010 *) context)->memory;				\
        memory[((address) & 0xffff) | 0xc000] = (x);			\
        memory[(((address) + 1) & 0xffff) | 0xc000 ] = (x) >> 8;	\
}
